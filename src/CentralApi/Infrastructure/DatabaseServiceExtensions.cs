namespace CentralApi.Infrastructure.Extensions
{
    using System;
    ...
    {
        public static IServiceCollection AddCentralApiDatabase(
            ...
            IConfiguration configuration)
        {
            ...
            services.AddDbContext<CentralApiDbContext>((sp, options) =>
            {
                var helper = sp.GetRequiredService<RdsIamAuthenticationHelper>();
                var logger = sp.GetRequiredService<ILogger<CentralApiDbContext>>();
                ...
                try
                {
                    ...
                    logger.LogInformation("CentralApiDbContext configured successfully");
                }
                catch (Exception ex)
                {
                    logger.LogError(ex, "Failed to configure CentralApiDbContext with RDS IAM authentication");
